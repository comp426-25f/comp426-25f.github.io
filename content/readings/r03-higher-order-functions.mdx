...
```ts title="TypeScript - Traditional Function"
...
On the top is `doubleNumber` as a traditional TypeScript function, and on the bottom is `doubleNumber` as an arrow function. Both are perfectly valid ways to declare the `doubleNumber` function. In addition, we can call both functions in the same way - using `doubleNumber(num)`.
...
```ts title="Variables in TypeScript"
...
```ts title="TypeScript - Arrow Function"
/** Function that doubles the input number */
let doubleNumber = (num: number): number => {
  return num * 2;
}
...
As you can see, we are creating a *variable* with the name `doubleNumber`, our function name. But, *our entire function is the value of this variable*! When we create the `doubleNumber` arrow function, we treat `(num: number): number => { return num * 2; }` as a value assigned to the name `doubleNumber`.
...
```ts title="Function Literal Syntax"
...
The type annotations for function literals *are based on their parameter types and return type*. The type annotation has the following syntax:
...
Sometimes, we can use *type aliases* from TypeScript reading (R02) to make working with function types a bit more readable:

```ts
// Define a type alias for our numerical operation function.
type NumberTransformer = (num: number) => number;

// Use the type annotation on our variable `doubleNumber`.
let doubleNumber: NumberTransformer = (num: number): number => {
    return num * 2;
}
```
...
```ts title="Number Transform Functions"
/** Function that doubles the input number */
let doubleNumber: (num: number) => number = (num: number): number => {
    return num * 2;
}
...
let mapNumbers = (
    nums: number[],
    transformFunction: (num: number) => number
): number[] => { ... }
```
...
To make our lives easier and make our code a lot more concise, let's bring back our type alias.
...
let mapNumbers = (
    nums: number[],
    transformerFunction: NumberTransformer
): number[] => { ... }
```

Finally, let's implement our new `mapNumbers` function!

```ts title="Implement mapNumbers With Function Parameter"
...
This is useful. As you can see, we modified our function to use the passed in `transformerFunction` to actually transform each number in our array. Let's see our new `mapNumbers` function in action:

```ts title="Use the mapNumbers Function"
...
```ts title="generateMultiplyTransformerFunction Header"
...
```ts title="generateMultiplyTransformerFunction Implementation"
